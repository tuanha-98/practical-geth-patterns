...
func vicPanicRecover(expected error, fn func()) (err error) {
	defer func() {
		panic := recover()
		if panic == nil {
...
func TestVicFeedPanics(t *testing.T) {
	// we will make multiple scope for multiple case
	{
		...
		if err := vicPanicRecover(expected, secondSend); err != nil {
			t.Error(err)
		}
	}
	{
	...
	{
		// sub with only read channel
		var f VicFeed
		if err := vicPanicRecover(errBadChannel, func() { f.Subscribe(make(<-chan int)) }); err != nil {
			t.Error(err)
		}
	}
	{
		// sub without channel
		var f VicFeed
		if err := vicPanicRecover(errBadChannel, func() { f.Subscribe(0) }); err != nil {
			t.Error(err)
		}
	}
}

func TestVicFeed(t *testing.T) {
	var feed Feed
...
func TestVicFeedSubscribeSameChannel(t *testing.T) {
	var (
		feed Feed
		done sync.WaitGroup
		ch   = make(chan int)
...
func TestVicFeedSubscribeBlockedPost(t *testing.T) {
	var (
		feed   Feed
		nsends = 2000
		ch1    = make(chan int)
...
func TestVicFeedUnsubscribeBlockedPost(t *testing.T) {
	var (
		feed   Feed
		nsends = 200
...
func TestVicFeedUnsubscribeFromInbox(t *testing.T) {
	var (
		feed Feed
		ch1  = make(chan int)
		ch2  = make(chan int)
...
func BenchmarkVicFeedSend1000(b *testing.B) {
	var (
		done  sync.WaitGroup
		feed  Feed
		nsubs = 1000
