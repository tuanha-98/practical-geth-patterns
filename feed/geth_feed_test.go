...
func panicRecover(expected error, fn func()) (err error) {
	defer func() {
		panic := recover()
		if panic == nil {
			err = fmt.Errorf("Send did not panic")
		} else if !reflect.DeepEqual(panic, expected) {
			err = fmt.Errorf("Got: %#v, expected error: %#v", panic, expected)
		}
	}()
	fn()
	return nil
...
func TestGethFeedPanics(t *testing.T) {
	// we will make multiple scope for multiple case
	{
		// send before send
		var f Feed
		// here feed will be initialized with channel type int
		f.Send(int(0))
		expected := feedTypeError{op: "Send", got: reflect.TypeOf(uint64(0)), want: reflect.TypeOf(int(0))}
		secondSend := func() {
			// here expect panic after type check uint64 != int
			f.Send(uint64(0))
		}
		if err := panicRecover(expected, secondSend); err != nil {
			t.Error(err)
		}
	}
	{
	...
	}
	{
		...
		var f Feed
		f.Send(int(0))
		expected := feedTypeError{op: "Subscribe", got: reflect.TypeOf(make(chan uint64)), want: reflect.TypeOf(make(chan<- int))}
		secondSend := func() {
	...
	{
		var f Feed
		if err := panicRecover(errBadChannel, func() { f.Subscribe(make(<-chan int)) }); err != nil {
			t.Error(err)
		}
	}
	{
		var f Feed
		if err := panicRecover(errBadChannel, func() { f.Subscribe(0) }); err != nil {
			t.Error(err)
		}
	}
}

func TestGethFeed(t *testing.T) {
	var feed Feed
...
func TestGethFeedSubscribeSameChannel(t *testing.T) {
	var (
		feed Feed
		done sync.WaitGroup
		ch   = make(chan int)
...
func TestGethFeedSubscribeBlockedPost(t *testing.T) {
	var (
		feed   Feed
		nsends = 2000
		ch1    = make(chan int)
...
func TestGethFeedUnsubscribeBlockedPost(t *testing.T) {
	var (
		feed   Feed
		nsends = 200
...
func TestGethFeedUnsubscribeSentChan(t *testing.T) {
	var (
		feed Feed
		ch1  = make(chan int)
		ch2  = make(chan int)
...
func TestGethFeedUnsubscribeFromInbox(t *testing.T) {
	var (
		feed Feed
		ch1  = make(chan int)
		ch2  = make(chan int)
...
func BenchmarkGethFeedSend1000(b *testing.B) {
	var (
		done  sync.WaitGroup
		feed  Feed
		nsubs = 1000
